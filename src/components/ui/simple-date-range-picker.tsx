      ...
      <Popover open={isOpen} onOpenChange={handleOpenChange}>
        <PopoverTrigger asChild>
          <Button
            ...
            variant={"outline"}
            className={cn(
              showPresets ? "w-[280px] sm:w-[320px]" : "w-full sm:w-[240px]",
              "justify-start text-left font-normal",
              showPresets && "bg-background border-2 shadow-lg hover:shadow-xl transition-all duration-200 hover:border-primary/50",
              !date && "text-muted-foreground"
            )}
          >
            <CalendarIcon className={cn(
              "flex-shrink-0",
              showPresets ? "mr-3 h-5 w-5 text-primary" : "mr-2 h-4 w-4"
            )} />
            {date?.from ? (
              showPresets ? (
        ...
        <PopoverContent 
          className={cn(
            "w-auto p-0",
            showPresets && "shadow-2xl border-2"
          )} 
          align="start"
        >
          {showPresets ? (
