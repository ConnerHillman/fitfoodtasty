...
const PackagesBar = ({ onSelect }: PackagesBarProps) => {
  ...
  const handlePackageClick = (pkg: MealPackage) => {
    // Navigate to the dedicated selection page
    navigate(`/packages/${pkg.id}/select`);
    // Also call onSelect if provided (for backwards compatibility)
    onSelect?.(pkg);
  };
    ...
    <section aria-labelledby="packages-heading" className="mb-8">
      <div className="text-center mb-6">
        ...
        <p className="text-muted-foreground text-sm md:text-base">
          We offer 3 sizes of meals - Regular, (LowCal), and MASSIVE! Select your package and then choose your meals.
        </p>
